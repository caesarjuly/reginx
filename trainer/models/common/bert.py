...
class BertEmbedding(tf.keras.layers.Layer):
    ...
    def __init__(self, vocab_size, length=128, dim=512, **kwargs):
        ...
        self.position_emb = tf.keras.layers.Embedding(input_dim=length, output_dim=dim)
        self.segment_emb = tf.keras.layers.Embedding(
            input_dim=2, output_dim=dim, mask_zero=True
        )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.emb = BertEmbedding(
            vocab_size=vocab_size, length=seq_length, dim=model_dim
        )
        self.encoders = [
            Encoder(
...
class BertNSP(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        emb = self.dropout(self.emb(inputs, training=training))
        # shape [batch_size, 2]
        return tf.squeeze(self.dense(emb[:, 0, :]), axis=[1])
